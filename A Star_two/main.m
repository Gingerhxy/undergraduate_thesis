...
load mapIEEE.mat                    % 加载地图
load pointIEEE.mat                  % 加载起止位置点
...
text(map.start(1)+.8,map.start(2)+.8,map.start(3)+.8,'S','FontSize',14,'FontWeight','bold');
% 绘制终止点
plot3(map.goal(1)+.5,map.goal(2)+.5,map.goal(3)+.5,'gd');hold on;
text(map.goal(1)+.8,map.goal(2)+.8,map.goal(3)+.8,'T','FontSize',14,'FontWeight','bold');
